<div>
    <form wire:submit="save" class="space-y-6">
        ...
        <div>
            ...
            <div class="space-y-4">
                <!-- GitHub設定情報 -->
                @if(auth()->user()->hasGitHubSettings())
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 text-green-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
                            </svg>
                            <span class="text-sm font-medium text-green-800">GitHub設定が完了しています</span>
                        </div>
                        <p class="mt-2 text-sm text-green-700">
                            GitHubオーナー: <code class="bg-green-100 px-2 py-0.5 rounded">{{ auth()->user()->getGitHubOwner() }}</code>
                        </p>
                        <p class="mt-1 text-sm text-green-700">
                            あなたのGitHub設定からトークンが自動的に使用されます。
                        </p>
                    </div>
                @else
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm font-medium text-yellow-800">GitHub設定が未完了です</span>
                        </div>
                        <p class="mt-2 text-sm text-yellow-700">
                            リポジトリを追加する前に、GitHub設定を完了してください。
                        </p>
                        <a href="{{ route('github.settings') }}" 
                           class="mt-3 inline-flex items-center px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white text-sm font-medium rounded-md transition-colors">
                            GitHub設定に移動
